    ...
    if "relations" in iou_types:
        if output_folder:
            ...
            with open(os.path.join(output_folder, "evaluation_res.txt"), 'w') as f:
                f.write(result_str)
        #print(float(np.mean(result_dict[mode + cfg.GLOBAL_SETTING.CHOOSE_BEST_MODEL_BY_METRIC][100])))
        return float(avg_metrics), 0
        #return float(np.mean(result_dict[mode + cfg.GLOBAL_SETTING.CHOOSE_BEST_MODEL_BY_METRIC][100])), 0
    else:
        return -1
